...
class DailyLedger {
    ...
    renderChart() {
        ...
        requestAnimationFrame(() => {
            requestAnimationFrame(() => {
                chartArea.querySelectorAll('.chart-bar-seg').forEach(seg => {
    ...
    // 스택 바 생성 (카테고리별 비율 한 줄)
    buildStackBar(sorted, total) {
        const segs = sorted.slice(0, 6).map(([cat, amount]) => {
            const pct = (amount / total) * 100;
            const color = this.getChartColor(cat);
            return `<div class="chart-bar-seg" style="background:${color}" data-pct="${pct.toFixed(1)}"></div>`;
        }).join('');
        return `<div class="chart-stack-bar">${segs}</div>`;
    }

    // 컴팩트 모드 (고정지출 펼쳐져 있을 때)
    renderChartCompact(chartArea, sorted, total) {
        const top = sorted.slice(0, 5);
        const legendHtml = top.map(([cat, amount]) => {
        ...
        chartArea.innerHTML = `
            <div class="chart-compact">
                ${this.buildStackBar(sorted, total)}
    ...
    renderChartDetailed(chartArea, sorted, total) {
        ...
        const items = [...top5];
        if (restTotal > 0) items.push(['기타', restTotal]);

        const rowsHtml = items.map(([cat, amount]) => {
            const pct = ((amount / total) * 100).toFixed(0);
            const color = this.getChartColor(cat);
            return `<div class="chart-row">
                <span class="chart-row-dot" style="background:${color}"></span>
                <span class="chart-row-name">${this.escapeHtml(cat)}</span>
                <span class="chart-row-amt">${Utils.formatCurrency(amount)}</span>
